...
public class FrmEspecialidades extends FrmIntern {
        ...
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (cnx.isOkConexion()) {
                    ...
                    EspecialidadesList = cnx.listEspecialidades("SELECT * FROM " + cnx.TABLA);

                    cnx.isCloseConexion();
                }
                progressEspecialidad.setMaximum(EspecialidadesList.size());
                ...
                for (int index = 0; index < EspecialidadesList.size(); index++) {
                    Especialidad l = EspecialidadesList.get(index);
                ...
                setTitle("Especialidadess...cantidad:" + EspecialidadesList.size());
    ...
    private boolean _isValidate(Especialidad l) {
        boolean isOk = false;

        if (l.getEspecialidad().trim().isEmpty()) {
    ...
    private Especialidad _getObrasocialForm() {
        Especialidad l = new Especialidad();

        l.setId(txtId.toEntero());
        l.setEspecialidad(txtEspecialidad.getText().trim());

        return l;
    }

    private void _setForm(Especialidad l) {
        txtId.setText(String.valueOf(l.getId()));
        txtEspecialidad.setText(l.getEspecialidad().trim());
    ...
    private void _selectionRow() {
        ...
        Especialidad l = new Especialidad(id, Especialidad);
    ...
    private boolean _isNew() {
        boolean isOk = false;
        Especialidad l = _getObrasocialForm();
        if (_isValidate(l)) {
            //Esta en codiciones
            Especialidades cnx = new Especialidades();
            if (cnx.isOkConexion()) {
